...
export class MainGroupComponent implements OnInit {
  displayedColumns: string[] = ['index', 'subGroupCode','subGroupName', 'subGroupFormationDate','chairperson', 'subGroupPhone']
  ...
  branchLookup(): void {
    ...
    dialogRef.afterClosed().subscribe(results => {
      ...
      this.formData.controls.mainGroupSolCode.setValue(results.data.solCode)
      this.formData.controls.mainGroupBranchName.setValue(this.dialogData.solDescription)
    })
  }
  mainGroupChairpersonLookup(): void {
    const dialogRef = this.dialog.open(RetailCustomerLookupComponent, {
    });
    dialogRef.afterClosed().subscribe(results => {
      this.dialogData = results.data;
      this.formData.controls.mainGroupChairperson.setValue(results.data.customerCode)
    })
  }
  mainGroupSecretaryLookup(): void {
    const dialogRef = this.dialog.open(RetailCustomerLookupComponent, {
    });
    dialogRef.afterClosed().subscribe(results => {
      this.dialogData = results.data;
      this.formData.controls.mainGroupSecretary.setValue(results.data.customerCode)
    })
  }
  mainGroupTreasurerLookup(): void {
    const dialogRef = this.dialog.open(RetailCustomerLookupComponent, {
    });
    dialogRef.afterClosed().subscribe(results => {
      this.dialogData = results.data;
      this.formData.controls.mainGroupTreasurer.setValue(results.data.customerCode)
    })
  }
  getPage() {
        ...
        if (this.function_type == "A-Add") {
          ...
          this.subscription = this.mainService.getMainGroupByCode(this.group_code).subscribe(
            res => {
              this.loading = false
              this.results = res
              this.getSubGroups(this.results.subGroups)
              // count 
              this.formData = this.fb.group({
                id: [this.results.id],
