...
export class HitcmService {

  headers = new HttpHeaders().set('Content-Type', 'application/json');
    // API endpoint
  baseURL = `${environment.apiUrl}/api/v1/parameters/configurations/interests/hitcm/`;
  ...
  changeMessage(message: string) {
    ...
    // Check if it exists
    checkHitcm(params: any): Observable<any> {
  ...
  // Add 
  createHitcm(data: any): Observable<any> {
  ...
  // Get by id
  getHitcmId(id: any): Observable<any> {
  ...
  getHitcmByHitcm(params:any): Observable<any> {
    let API_URL = `${this.baseURL}/find/by/hitcm/`;
    return this.http.get(API_URL, { params:params, withCredentials: false })
      .pipe(
        map((res) => {
          return res || {}
  ...
  updateHitcm(Hitcm: string | null, data: any): Observable<any> {
    let API_URL = `${this.baseURL}/update/${Hitcm}`;
    return this.http.put(API_URL, data, {headers: this.headers, withCredentials: false})
      .pipe(
        catchError(this.errorMgmt)
      )
  }
  deleteHitcm(id: any): Observable<any> {
