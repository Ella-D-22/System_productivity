...
export class HbivsmService {


  headers = new HttpHeaders().set('Content-Type', 'application/json');
    // API endpoint
  baseURL = `${environment.apiUrl}/api/v1/parameters/configurations/interests/hbivsm`;
  ...
  changeMessage(message: string) {
    ...
    // Check if it exists
    checkhbivsm(params: any): Observable<any> {
      ...
      return this.http.post(API_URL, {}, {params:params, headers: this.headers, withCredentials: false }).pipe(map(res => {
        console.log("data responded", res)
          return res || {}
        }),
  ...
  // Add 
  createhbivsm(data: any): Observable<any> {
  ...
  // Get by id
  gethbivsmId(id: any): Observable<any> {
  ...
  gethbivsmByhbivsm(int_tbl_code:any): Observable<any> {
    let API_URL = `${this.baseURL}/find/by/hbivsm/${int_tbl_code}`;
    return this.http.get(API_URL, { withCredentials: false })
      .pipe(
        map((res) => {
          return res || {}
  ...
  updatehbivsm(id: any, data: any): Observable<any> {
  ...
  deletehbivsm(id: any): Observable<any> {
