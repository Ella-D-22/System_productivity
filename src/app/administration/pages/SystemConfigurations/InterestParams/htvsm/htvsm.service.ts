...
export class HtvsmService {

  headers = new HttpHeaders().set('Content-Type', 'application/json');
    // API endpoint
  baseURL = `${environment.apiUrl}/api/v1/parameters/configurations/interests/htvsm/`;
  ...
  changeMessage(message: string) {
    ...
    // Check if it exists
    checkHtvsm(params: any): Observable<any> {
  ...
  // Add 
  createHtvsm(data: any): Observable<any> {
  ...
  // Get by id
  getHtvsmId(id: any): Observable<any> {
  ...
  getHtvsmByHtvsm(int_tbl_code:any): Observable<any> {
    let API_URL = `${this.baseURL}/find/by/htvsm/${int_tbl_code}`;
    return this.http.get(API_URL, {withCredentials: false })
      .pipe(
        map((res) => {
          return res || {}
  ...
  updateHtvsm(Htvsm: string | null, data: any): Observable<any> {
    let API_URL = `${this.baseURL}/update/${Htvsm}`;
    return this.http.put(API_URL, data, {headers: this.headers, withCredentials: false})
      .pipe(
        catchError(this.errorMgmt)
      )
  }
  deleteHtvsm(id: any): Observable<any> {
