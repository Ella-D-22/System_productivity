...
import { HtvsmService } from '../htvsm/htvsm.service';

@Component({
  selector: 'app-htvsm',
  templateUrl: './htvsm.component.html',
  styleUrls: ['./htvsm.component.scss']
})
export class HtvsmComponent implements OnInit {
  ...
  constructor(
    ...
    submitted = false;
      ...
      disabledFormControll(){
        ...
        this.formData.controls.cr_crt_p_mnth.disable();
        this.formData.controls.cr_crt_p_dys.disable();
        this.formData.controls.cr_run_p_mnth.disable();
        this.formData.controls.cr_run_p_dys.disable();
        this.formData.controls.cr_pr_ind.disable();
        this.formData.controls.cr_max_slab_amt.disable();
        this.formData.controls.cr_amt_ind.disable();
        this.formData.controls.cr_normal.disable();
        this.formData.controls.cr_penal.disable();
        this.formData.controls.cr_extension.disable();
        this.formData.controls.record_del.disable();
      }

      getPage(){
        this.subscription = this.htvsmAPI.currentMessage.subscribe(message =>{
        ...
        if(this.function_type == "A-Add"){
          ...
          this.subscription = this.htvsmAPI.getHtvsmByHtvsm(this.int_tbl_code).subscribe(res=>{
            this.results = res;
            this.formData = this.fb.group({
              function_type: [this.function_type],
              int_tbl_code: [this.int_tbl_code],
              ...
              cr_crt_p_mnth: [this.results.cr_crt_p_mnth],
              cr_crt_p_dys: [this.results.cr_crt_p_dys],
              cr_run_p_mnth: [this.results.cr_run_p_mnth],
              cr_run_p_dys: [this.results.cr_run_p_dys],
              cr_pr_ind: [this.results.cr_pr_ind],
              cr_max_slab_amt: [this.results.cr_max_slab_amt],
              cr_amt_ind: [this.results.cr_amt_ind],
              cr_normal: [this.results.cr_normal],
              cr_penal: [this.results.cr_penal],
              cr_extension: [this.results.cr_extension],
          ...
          // call to disable edit
          this.subscription = this.htvsmAPI.getHtvsmId(this.params).subscribe(res=>{
      ...
      onSubmit() {
          ...
          if (this.formData.valid) {
            if(this.function_type == "A-Add"){
            this.subscription = this.htvsmAPI.createHtvsm(this.formData.value).subscribe(res=>{
              this.results = res;
              ...
              this.subscription = this.htvsmAPI.updateHtvsm(this.eventId, this.formData.value).subscribe(res=>{
                this.results = res;
                  this._snackBar.open("Executed Successfully!", "X", {
                    horizontalPosition: this.horizontalPosition,
                    verticalPosition: this.verticalPosition,
